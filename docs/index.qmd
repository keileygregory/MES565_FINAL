...
hbk_summary <- hbk %>%
  select("Glider_depth_m", "Water_temperature_C", "Salinity_PSU", "Oxygen_concentration_µmol_L", "RASTERDEPTH_m", "RASTERSLOPE_deg", "NEAR_DIST") %>%
  pivot_longer(
    cols = everything(),
  ...
  mutate(
    Oceanographic_Variable = case_when(
      ...
      Oceanographic_Variable == "NEAR_DIST" ~ "Distance to Nearest Shoreline (m)",
...
# Export as CSV
write_csv(bkg_summary, "data/output_summary_tables/bkg_summary.csv")
```

```{r, include=FALSE}
# Join hbk detection and bkg background mission summaries by Oceanographic_Variable col
join_summary <- hbk_summary %>%
  left_join(bkg_summary %>% 
            ...
            by = "Oceanographic_Variable"
            )
```

```{r, include=FALSE}
# Remove glider depth row (not included bc not really an oceanographic variable), calculate new `Difference in Means` col, rename Oceanographic_Variable col, and remove "n" and "BKG_Mean" columns + set col order
...
bkg_long <- bkg %>%
  ...
  mutate(
    Oceanographic_Variable = factor(
      Oceanographic_Variable,
      levels = levels(hbk_long$Oceanographic_Variable)  # *same order as detections
    )
  ) %>%
  filter(!is.na(Value)) # drop non-finite values
```
